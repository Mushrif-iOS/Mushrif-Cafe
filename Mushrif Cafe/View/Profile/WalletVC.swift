    ...
    private func getWalletDetails() {
        ...
        APIManager.shared.getCallWithParams(APPURL.wallet_details, params: aParams) { responseJSON in
            ...
            DispatchQueue.main.async {
                ...
                self.balanceLabel.text = UserDefaultHelper.language == "en" ? "\(doubleValue.rounded(toPlaces: 3)) \("kwd".localized())" : "\("kwd".localized()) \(doubleValue.rounded(toPlaces: 3))"
...
extension WalletVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if dict.txnType == "Cr" {
            ...
            cell.amtLabel.text = UserDefaultHelper.language == "en" ? "+ \(doubleValue.rounded(toPlaces: 3)) \("kwd".localized())" : "\("kwd".localized()) \(doubleValue.rounded(toPlaces: 3)) +"
        } else {
