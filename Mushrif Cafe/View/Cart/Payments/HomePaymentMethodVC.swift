...
class HomePaymentMethodVC: UIViewController, Instantiatable {
    ...
    override func viewDidLoad() {
        ...
        self.keepLabel.text = UserDefaultHelper.language == "en" ? "\("wallet".localized()) \("balance".localized()): (\(doubleValue.rounded(toPlaces: 3)) \("kwd".localized()))" : "\("wallet".localized()) \("balance".localized()): (\("kwd".localized()) \(doubleValue.rounded(toPlaces: 3)))"
    }
